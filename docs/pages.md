...
**EXTREME FUN MODE (`FUN_MODE_EXTREME_COOKIE = "smse_fun_mode_extreme"`, default off).** A second opt-in tier layered on top of Fun Mode, exposed under the same Settings section. Invariant: *extreme implies fun*. Enforced in three places so the two prefs can never drift: (1) the server helper `setFunModeCookie(false)` cascades and clears the extreme cookie; (2) `setFunModeExtremeCookie(true)` refuses when Fun Mode is off; (3) `getFunModeExtreme()` returns `false` when Fun Mode is off, even if the raw cookie still reads `1`. The toggle itself is in `components/FunModeExtremeToggle.tsx` — a single pill button with an always-running rainbow gradient (`@keyframes smse-rainbow-scroll` in `app/globals.css`), a conic-gradient aura pseudo-element that spins + pulses around the button when on (`smse-rainbow-aura-spin` + `smse-rainbow-aura-pulse`), and disabled state when Fun Mode is off. `prefers-reduced-motion: reduce` collapses the animations to static gradients.
