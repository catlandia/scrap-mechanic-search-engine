...
6. **Inserts go in with `creators: []` and `creators_refreshed_at: null`.** Multi-creator attribution is not scraped during ingest — the Workshop HTML scrape costs 200–800 ms per row in the happy path and 30–60 s per row when Steam 429s, and a manual run that produces hundreds of inserts in one go would blow the 300 s function budget (every run from 2026-04-24 onward got stuck with `ended_at=null` for exactly this reason). The null `creators_refreshed_at` floats new rows to the front of `refreshStaleCreators`'s `ORDER BY ... ASC NULLS FIRST` rotation, so the daily ingest cron's 200-row top-up and the weekly refresh's 500-row drain backfill attribution within a day. Single-item paths (`addCreation`, `submitCreation`, `rescrapeCreatorsAction`) still scrape inline because they're capped at one row and the user is waiting on the result.
