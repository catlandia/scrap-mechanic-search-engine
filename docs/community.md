...
**Delete confirmation:** The client-side "delete" button is a two-stage toggle, not a one-click kill. First tap on "delete" swaps the link for a red **"confirm delete?"** pill + a **"cancel"** link sibling; only the second tap on confirm fires `deleteComment`. State lives on the parent `CommentSection` as `confirmingDeleteId`, clears on successful delete. Prevents fat-finger taps on phones and accidental double-clicks on desktop.
...
- Must not be banned
- Must not be muted
- Steam account must be ≥ 7 days old (prevents fresh sock-puppet accounts). If the user's Steam profile is private and hides `timecreated`, the gate **blocks them** (treated as unknown-age) unless the creator has flipped `bypassAgeGate` on their user row via `/admin/users`. Pre-V5.5 the null case leaked through — that was the bug several people pointed out. Legit users stuck on this gate can plead their case via `/verify/appeal` (added V7.1), which broadcasts to the moderator tier with their message.
