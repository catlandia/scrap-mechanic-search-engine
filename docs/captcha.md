...
Images live in `lib/captcha/images/` as opaque numbered files (`01.jpg` – `25.jpg`). They are **intentionally outside `public/`** so there's no direct URL like `/captcha/Mechanic1.jpg` to curl. The character → filename mapping is server-side only in `lib/captcha/config.ts`. On Cloudflare the bytes are served as content-hashed static assets proxied server-side through the gated route — see *Runtime delivery* below.
...
**Runtime delivery: content-hashed static assets via the `ASSETS` binding.** Free Cloudflare Workers cap a script at 3 MiB gzipped, so the old base64 JSON manifest (which inlined the full 23 MB of images straight into the route bundle) had to go. `scripts/fetch-captcha-images.ts` now copies each jpg to `/public/_captcha/<sha256>.jpg` (gitignored) and writes a tiny `lib/captcha/_images.manifest.json` mapping `01.jpg → <hash>.jpg`. `/api/captcha/image` (and the scrapcha sibling) stay session-gated: each looks up the hash for the current question and fetches the bytes server-side from the `ASSETS` binding, streaming them back with `Cache-Control: no-store`. The client never sees a stable per-image URL, so the set can't be enumerated — the hashed files are reachable only by an unguessable 64-hex name that is never linked anywhere. Under `next dev` (no binding) the route falls back to reading the original file off disk.
