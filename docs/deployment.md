...
1. Writes a row to `deploy_announcements` with `scheduled_at = now() + 75s`. The extra ~15 s over the original 60 s gives a visitor mid-action (finishing a comment, saving a triage decision, submitting a form) enough slack that the countdown doesn't race the push.
